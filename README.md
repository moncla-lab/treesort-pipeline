...
## Running the pipeline locally and using example data:
...
## Running the pipeline on AWS and using example data:

Perform preparation for running locally as described above. In addition, you will need a copy of a bash script that has a few critical environment variables in order to submit jobs to AWS. These variables are as sensitive as passwords and should remain private. Please do not distribute this file or commit it to this repository.

To prepare your command line to interact with AWS, run:

```
source /path/to/aws/environment/file.sh
```

You can check that the appropriate variables `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_DEFAULT_REGION`, `AWS_BATCH_JOB_DEFINITION`, and `AWS_BATCH_JOB_QUEUE` are present in your environment by running `env | grep AWS`.

A local environment needs to be installed so that you have the tools necessary to interact with AWS on your local machine. To install and activate, run

```
conda env create -f local-environment.yml
conda activate treesort-local
```

Then, to run the pipeline, run
...
+ `target_tree`: original **rooted** backbone tree (before TreeSort was run on it) provided in `rule files`
