...
def compute_rea_rate_simple(gene_segments=None, annotated_tree=None, evol_rate=None, ignore_top_edges=1):
	...
	if gene_segments:
		segment_counts = {seg: 0 for seg in gene_segments}
	...
	for node in annotated_tree.Objects:
		...
		if node.traits.get('is_reassorted'):
			...
			rea_events += 0.5 if is_uncertain else 1 # overall rate
			
			if gene_segments:

				segs = [g.split("(")[0] for g in rea_annotation.split('-')]
				for seg in segs:
					clean_seg = seg.lstrip('_')	  # actual segment name without underscore
					
					if clean_seg in segment_counts:
						if seg.startswith('_'):
							segment_counts[clean_seg] += 0.5   # uncertain segment
						else:
							segment_counts[clean_seg] += 1.0 # certain segment
	...
	if gene_segments:
		...
		segment_rates = {
			seg: (segment_counts[seg] / tree_length * evol_rate)
			if tree_length > 0 else 0.0
			for seg in gene_segments
		}
		
		return rea_rate, segment_rates
...
def compute_rea_rate_binary_mle(annotated_tree, evol_rate, backbone_seg):
	...
	est = compute_rea_rate_simple(None, annotated_tree, evol_rate, ignore_top_edges=1)
	np_est = np.array([est])
...
# first, using the summary tree, calculate the molecular clock
clock_rate, r_val = estimate_clock_rate(args.summary_tree, args.backbone_aln)

# next, make the list of gene segments that a rea rate will be calculated for

gene_segments = ['HA', 'NA', 'NP', 'NS', 'PB1', 'PB2', 'MP', 'PA'] 
gene_segments = [g for g in gene_segments if g != args.backbone.upper()]

rows = []

# calculate overall and per segment rate
...
for rep_tree in args.trees:
	...
	simple_rate, seg_rates = compute_rea_rate_simple(gene_segments, tree, clock_rate, ignore_top_edges=1)
